      ...
      <div className="grid md:grid-cols-2 gap-6">
        {semester.subjects.map(subj => {
          const lectureTotal = subj.chapters.length;
            const lectureDone = subj.chapters.filter(c => lectures[c.id]).length;
            const exerciseDone = subj.chapters.filter(c => exercises[c.id]).length;
            return (
              <div key={subj.id} className="rounded-lg border p-4 space-y-2 bg-white dark:bg-gray-900">
                <h2 className="font-medium text-sm">{subj.name}</h2>
                <ProgressBar label="Lectures" done={lectureDone} total={lectureTotal} />
                <ProgressBar label="Exercises" done={exerciseDone} total={lectureTotal} />
              </div>
            );
        })}
      </div>
...
function ProgressBar({ label, done, total }: { label: string; done: number; total: number }) {
  ...
  return (
    <div className="space-y-1">
      <div className="flex justify-between text-xs"><span>{label}</span><span>{done}/{total} ({pct}%)</span></div>
